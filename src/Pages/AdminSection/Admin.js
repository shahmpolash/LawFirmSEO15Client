...
const Admin = () => {
    ...
    useEffect(() => {
        fetch(`https://legelserviceseo15-4512b38069b4.herokuapp.com/subscribers`)
            .then((res) => res.json())
            .then((info) => setSubscribers(info.reverse())); // Reverse the array initially
    }, []);

    useEffect(() => {
        fetch(`https://legelserviceseo15-4512b38069b4.herokuapp.com/orders`)
            .then((res) => res.json())
                                        ...
                                        <tbody>
                                            {currentOrders.map((order, index) => (
                                                <tr key={order._id}>
                                                    ...
                                                    <td><Link className=' btn btn-primary mb-5' to={`/admin/payment-status/${order._id}`}>Update Order</Link></td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
